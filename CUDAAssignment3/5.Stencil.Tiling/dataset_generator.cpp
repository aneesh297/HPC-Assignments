  ...
  for (int i = 0; i < height; ++i) {
    for (int j = 0; j < width; ++j) {
      for (int k = 0; k < depth; ++k) {
        ...
        //value(data,i,j,k) = (rand() % (128 - 32)) + 32;
      }
    }
  }
  return data;
...
void printAscii(unsigned char* a,int n){
  for(int i=0;i<n;i++) printf("%d ",(int)*a);
  printf("\n");
}
...
static void create_dataset(int datasetNum, int width, int height, int depth) {
  ...
  char output_file_name[] = "expected_output.ppm";
